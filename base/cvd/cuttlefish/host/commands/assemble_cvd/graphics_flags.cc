...
Result<bool> SelectGpuVhostUserMode(const GpuMode gpu_mode,
  ...
  if (gpu_vhost_user_mode_arg == kGpuVhostUserModeAuto) {
    if (gpu_mode == GpuMode::GuestSwiftshader) {
      VLOG(1) << "GPU vhost user auto mode: not needed for --gpu_mode="
              << GpuModeString(gpu_mode) << ". Not enabling vhost user gpu.";
      return false;
    }

    if (!VmManagerIsCrosvm(vmm)) {
      VLOG(1) << "GPU vhost user auto mode: not yet supported with " << vmm
    ...
    if (host_arch == Arch::Arm64) {
      VLOG(1) << "GPU vhost user auto mode: detected arm64 host. Enabling "
                 "vhost user gpu.";
      return true;
    }

    VLOG(1) << "GPU vhost user auto mode: not needed. Not enabling vhost "
               "user gpu.";
    return false;
  }
