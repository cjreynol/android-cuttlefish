...
template <typename T>
Result<FlagValues<T>> FromGlobalGflags(
...
template Result<FlagValues<bool>> FromGlobalGflags<bool>(
    const gflags::CommandLineFlagInfo& flag_info, const std::string& flag_name,
    std::function<Result<bool>(const std::string& value,
                               const std::string& name)>
        parse_func);
template Result<FlagValues<int>> FromGlobalGflags<int>(
    const gflags::CommandLineFlagInfo& flag_info, const std::string& flag_name,
    std::function<Result<int>(const std::string& value,
                              const std::string& name)>
        parse_func);
template Result<FlagValues<std::string>> FromGlobalGflags<std::string>(
    const gflags::CommandLineFlagInfo& flag_info, const std::string& flag_name,
    std::function<Result<std::string>(const std::string& value,
                                      const std::string& name)>
        parse_func);

Result<FlagValues<bool>> BoolFromGlobalGflags(
    const gflags::CommandLineFlagInfo& flag_info,
    const std::string& flag_name) {
  return CF_EXPECT(FromGlobalGflags<bool>(flag_info, flag_name, ParseBool));
}

Result<FlagValues<int>> IntFromGlobalGflags(
    const gflags::CommandLineFlagInfo& flag_info,
    const std::string& flag_name) {
  return CF_EXPECT(FromGlobalGflags<int>(flag_info, flag_name, ParseInt));
}

Result<FlagValues<std::string>> StringFromGlobalGflags(
    const gflags::CommandLineFlagInfo& flag_info,
    const std::string& flag_name) {
  return CF_EXPECT(
